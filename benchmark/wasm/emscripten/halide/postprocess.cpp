...
class PostprocessTask : public Halide::Generator<PostprocessTask> {
 public:
  ...
  Halide::GeneratorInput<Halide::Buffer<std::uint8_t>> background{"background",
                                                                  3};
  ...
  void generate() {
    ...
    output(x, y, c) = Halide::select(softmax(x, y) > threshold, input(x, y, c),
                                     background(x, y, c));
  }
  ...
  void schedule() {
    ...
    background.set_estimates({{0, 1920}, {0, 1080}, {0, 4}});
    inference.set_estimates({{0, 1920}, {0, 1080}, {0, 4}});
    threshold.set_estimate(0.5);
